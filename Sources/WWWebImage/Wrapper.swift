...
public extension WWWebImage.Wrapper {
    ...
    /// 
    /// 工作流程：
    /// 1. 獲取或創建 DownloadManager（透過 DownloadTaskManager）
    /// 2. 取消之前的下載（如果存在）
    ...
    func download(urlString: String, `default`: UIImage? = nil) async throws {
        ...
        try await cacheImage(with: urlString, manager: manager, defaultImage: `default`)
    }
...
private extension WWWebImage.Wrapper {
    ...
    /// 獲取或創建 DownloadManager，並設置下載狀態
    /// - Parameter urlString: 要下載的圖片網址
    /// - Returns: DownloadManager（用於追蹤下載狀態）
    ///
    /// 工作流程：
    /// 1. 從 DownloadTaskManager 獲取 ImageView 的 DownloadManager
    /// 2. 取消之前的下載（如果存在）
    /// 3. 設置期望 URL（expectedURL）
    /// 4. 重置取消狀態（isCancelled = false）
    /// 5. 返回 DownloadManager
    ///
    /// 關鍵：
    /// - 每次 download() 都會先取消舊任務
    /// - 設置新的 expectedURL
    /// - 舊任務完成時，shouldShowImage 返回 false，不顯示圖片
    func prepareDownloadManager(urlString: String) -> WWWebImage.DownloadManager? {
        
        let manager = WWWebImage.DownloadTaskManager.shared.manager(for: imageView)
    ...
    /// - Parameters:
    ///   - urlString: 圖片網址
    ///   - manager: DownloadManager（保存 task）
    ...
    /// 6. Cache 圖片並顯示
    /// 7. 如果下載失敗，保持預設圖
    func cacheImage(with urlString: String, manager: WWWebImage.DownloadManager, defaultImage: UIImage?) async throws {
